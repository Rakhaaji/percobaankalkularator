...
def get_sample_size(lot_size, inspection_level):
    ...
    general_level = {'I': [2, 8, 20, 50, 125, 315, 800, 2000, 5000, 12500, 31500], 'II': [2, 8, 20, 50, 125, 315, 800, 2000, 5000, 12500, 31500], 'III': [2, 8, 20, 50, 125, 315, 800, 2000, 5000, 12500, 31500]}
    
    special_level = {'S-1': [2, 3, 5, 8, 13, 20, 32, 50, 80, 125, 200], 'S-2': [2, 3, 5, 8, 13, 20, 32, 50, 80, 125, 200], 'S-3': [2, 3, 5, 8, 13, 20, 32, 50, 80, 125, 200], 'S-4': [2, 3, 5, 8, 13, 20, 32, 50, 80, 125, 200]}
    
    # Rentang ukuran lot
    lot_ranges = [(2, 8), (9, 15), (16, 25), (26, 50), (51, 90),(91, 150), (151, 280), (281, 500), (501, 1200),(1201, 3200), (3201, 10000), (10001, 35000),(35001, 150000), (150001, 500000), (500001, float('inf'))]
    
    # Cari indeks yang sesuai berdasarkan ukuran lot
    index = 0
    for i, (low, high) in enumerate(lot_range):
...
def aql_calculator(lot_size, inspection_level, aql_level):
    ...
    aql_limits = {0.010: (0, 1),0.015: (0, 1), 0.025: (0, 1),0.040: (0, 1),0.065: (0, 1),0.10: (0, 1),0.15: (0, 1),0.25: (0, 2),0.40: (0, 3),0.65: (0, 4),1.0: (0, 7),1.5: (0, 10),2.5: (0, 14),4.0: (0, 20),6.5: (0, 30)}
    ...
    return {"Sample Size": sample_size,"Accept Point": accept_point,"Reject Point": reject_point,"Inspection Level": inspection_level}

# Antarmuka Streamlit
def main():
    st.title("Kalkulator AQL dengan GIL (Tingkat Inspeksi Umum/Khusus)")
    
    st.markdown("""Kalkulator ini membantu menentukan ukuran sampel dan kriteria penerimaan berdasarkan:
    - Ukuran lot, 
    ...
    with col2:
        inspection_level = st.selectbox("Tingkat Inspeksi",
            options=['I', 'II', 'III', 'S-1', 'S-2', 'S-3', 'S-4'],
            index=1)
    
    with col3:
        aql_level = st.selectbox("Level AQL (%)",options=[0.010, 0.015, 0.025, 0.040, 0.065, 0.10, 0.15, 0.25,0.40, 0.65, 1.0, 1.5, 2.5, 4.0, 6.5], index=11)
